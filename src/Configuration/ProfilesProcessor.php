...
final class ProfilesProcessor
{
    ...
    public function process(array $profiles)
    {
        ...
        foreach ($profiles as $name => $profile) {
            ...
            $this->profiles[$name]['configurators'] = array_unique($this->profiles[$name]['configurators']);
        }
    ...
    private function processProfile($name, array $profile)
    {
        if (!empty($profile['import'])) {
            ...
            $resolvedProfile = ['generators' => [], 'configurators' => [], 'defaults' => [], 'import' => []];
        }
        ...
        if (isset($profile['configurators'])) {
            $resolvedProfile['configurators'] = array_merge($resolvedProfile['configurators'], $profile['configurators']);
        }
    ...
    {
        $profile = ['generators' => [], 'configurators' => [], 'defaults' => [], 'import' => $imports];
        ...
        foreach ($imports as $import) {
            ...
            $profile['configurators'] = array_merge($profile['configurators'], $processProfile['configurators']);
